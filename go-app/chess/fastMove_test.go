...
func Test_MoveSimple(t *testing.T) {
    ...
    board.On("getVulnerables2", white).Return(vulnerable, nil)
    ...
    board.On("setVulnerables2", white, Vulnerable{Point{-1, -1}, Point{-1, -1}}).Return(nil)
    ...
    board.On("setVulnerables2", white, vulnerable).Return(nil)
    err = simpleMove.undo()
    assert.Nil(t, err)
...
func Test_MoveCastle(t *testing.T) {
	...
	board.On("setEnPassant", white, EnPassant{Point{-1, -1}, Point{-1, -1}}).Return(nil)
    board.On("setVulnerables2", white, newVulnerable).Return(nil)
